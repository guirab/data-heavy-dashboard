...
| Reproduce | `pnpm data:download` fetches the 33 zips listed in [`data/manifest.json`](data/manifest.json) (URL, size, `Last-Modified`, SHA-256 each); `pnpm data:build` runs the pipeline in ~75 s; `pnpm data:verify` rebuilds and fails if any byte of `public/data/` changes. CI does this on every push, so the committed artifacts are provably what the code produces. |
